	...
	if( NULL != RBLE_GAP_Info.Handler){
		...
		switch(evt_id){
			...
			case RBLE_EVT_GAP_CONNECTION_COMP:
				...
				RBLE_DATA_GET8( Data_p, RBLE_GAP_Event.param.conn_comp.connect_info.reserved );
				...
				Data_p += ( sizeof( uint8_t ) * RBLE_BD_ADDR_LEN );
				RBLE_DATA_GET8( Data_p, RBLE_GAP_Event.param.conn_comp.connect_info.reserved2 );
...
void rBLE_print(char *str)
{
    uint16_t len;
    len = (uint16_t)strlen(str);
	com_srv_send( str , len );
}

int8_t debug_disp[100];
...
//	sprintf( debug_disp, "[Packet Type ]  : 0x%x \n", pnt->packet_type);
	sprintf( debug_disp, "[Type]0x%x \n", pnt->packet_type);
	rBLE_print( debug_disp );
//	sprintf( debug_disp, "[Packet Length] : 0x%x \n", pnt->packet_length);
	sprintf( debug_disp, "[Len]0x%x \n", pnt->packet_length);
	rBLE_print( debug_disp );
...
//	sprintf( debug_disp, "[Opecode] : 0x%x \n", pnt->opcode);
	sprintf( debug_disp, "[Ope]0x%x \n", pnt->opcode);
	rBLE_print( debug_disp );

	if(pnt->packet_length == 0){
		rBLE_print("[Data]None\n\n" );
	}else{
		rBLE_print( "[Data]\n" );
//		rBLE_print( "|0x00|0x01|0x02|0x03|0x04|0x05|0x06|0x07|0x08|0x09|0x0A|0x0B|0x0C|0x0D|0x0E|0x0F|\n" );
//		rBLE_print( "|-------------------------------------------------------------------------------|\n" );
		rBLE_print(" " );
#if 1
//		for(cnt=0,sub_cnt=1;cnt < pnt->packet_length;cnt++,sub_cnt++){
		for(cnt=0,sub_cnt=1;cnt < 1 ;cnt++,sub_cnt++){
			sprintf( debug_disp, "0x");
			rBLE_print( debug_disp );
			...
			if(sub_cnt == 16){
				sprintf( debug_disp, "%x\n",pnt->data[cnt]);
				rBLE_print( debug_disp );

				rBLE_print(" " );
				sub_cnt = 0;
			}else if(cnt == ( (pnt->packet_length) - 1 )){
				sprintf( debug_disp, "%x",pnt->data[cnt]);
				rBLE_print( debug_disp );
...
void rBLE_Data_Disp(RBLE_HEADER_t *pnt)
{
	...
	fprintf( stdout, "[Packet Type ]  : 0x%x \n", pnt->packet_type);
	fprintf( stdout, "[Packet Length] : 0x%x \n", pnt->packet_length);
	...
	fprintf( stdout, "[Opecode]       : 0x%x \n", pnt->opcode);
	if(pnt->packet_length == 0){
		fputs("[Data]          : None\n\n", stdout );
	}else{
		fputs( "[Data]\n", stdout );
		fputs( "|0x00|0x01|0x02|0x03|0x04|0x05|0x06|0x07|0x08|0x09|0x0A|0x0B|0x0C|0x0D|0x0E|0x0F|\n", stdout );
		fputs( "|-------------------------------------------------------------------------------|\n", stdout );
		fputs(" ", stdout );
